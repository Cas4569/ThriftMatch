...
export default async function WomenswearOuterwear() {
  const q = query(
      ...
      <div className="relative z-10 mx-auto max-w-7xl">
        ...
        <div className="mb-12">
          ...
          <div className="relative">
            ...
            {/* Purple → Pink accent */}
            <div className="mt-3 h-1 w-32 rounded-full bg-gradient-to-r from-[#7e22ce] via-[#f09acb] to-transparent shadow-[0_0_18px_rgba(240,154,203,0.55)]" />
        ...
        <div className="mb-6 flex items-end justify-between">
          ...
          <p className="hidden text-[10px] font-bold uppercase tracking-[0.2em] text-[#d886b1] sm:block">
        ...
        <section className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          ...
          {products.map((product, index) => (
            <div
              key={product.id}
              className="group relative overflow-hidden rounded-[2rem] border border-[#45245d] bg-[#140a1c]/90 shadow-[0_15px_45px_rgba(0,0,0,0.4)] backdrop-blur-xl transition duration-500 hover:-translate-y-3 hover:border-[#f09acb] hover:shadow-[0_25px_70px_rgba(126,34,206,0.3)]"
            >

              {/* Neon edge */}
              <div className="pointer-events-none absolute inset-0 z-20 rounded-[2rem] border border-transparent transition duration-500 group-hover:border-[#f09acb]/40" />

              {/* Product number */}
              <div className="absolute left-5 top-5 z-30 rounded-full border border-[#f09acb]/50 bg-[#13091b]/85 px-3 py-1.5 text-[9px] font-black tracking-[0.2em] text-[#f3afd0] backdrop-blur-md">
                {String(index + 1).padStart(2, "0")}
              </div>

              {/* Image */}
              <div className="relative h-[28rem] overflow-hidden bg-[#32174D]">
                ...
                {/* Product tag */}
                {product.tag && (
                  <div className="absolute left-5 top-16 rounded-full border border-[#f09acb]/50 bg-[#13091b]/85 px-4 py-2 text-[9px] font-bold uppercase tracking-[0.2em] text-[#f3afd0] backdrop-blur-md">
                    {product.tag}
                  </div>
                )}
              ...
              <div className="p-7">
                ...
                <h2 className="text-2xl font-black tracking-tight transition duration-300 group-hover:text-[#f5b4d7]">
                  {product.name}
                </h2>
                ...
                <div className="mt-5 flex items-center justify-between gap-4">
                  ...
                  <p className="text-xl font-black text-[#f09acb]">
                    {product.price}
                  </p>

                  <a
                    href={`/marketplace/product/${product.id}`}
                    className="rounded-full border border-[#5b3180] px-5 py-2.5 text-sm font-bold text-[#d8b4fe] transition duration-300 hover:border-[#f09acb] hover:bg-[#32174D] hover:text-[#f5b4d7] hover:shadow-[0_0_20px_rgba(240,154,203,0.2)]"
                  >
                    View →
                  </a>
        ...
        <div className="mt-14 flex flex-col gap-3 border-t border-[#32174D] pt-5 text-[9px] font-bold uppercase tracking-[0.25em] text-[#65477a] sm:flex-row sm:items-center sm:justify-between">
          ...
          <span>
            THRIFTMATCH // WOMENSWEAR_OUTERWEAR_DATABASE
          </span>
